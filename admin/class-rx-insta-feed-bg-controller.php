...
/**
 * The Rx_Insta_Feed_Controller class file that
 * control the feed
 *
 * @link       https://rextheme.com
 * @since      2.0.0
 *
 * @package    Rx_Insta_Feed_Controller
 * @subpackage Rx_Insta_Feed/admin
 */


class Rx_Insta_Feed_Controller {
    ...
    protected static function get_feed_queue() {
        ...
        return null != get_option('rx_insta_feed_media_feed_queue') ? get_option('rx_insta_feed_media_feed_queue') : array();
    }
    ...
    public static function add_id_to_feed_queue( $feed_id ) {
        ...
        if( ! in_array( $feed_id, $feed_queue_ids ) ) {
            ...
            update_option( 'rx_insta_feed_media_feed_queue', $feed_queue_ids );
        }
    }
    ...
    public static function remove_id_from_feed_queue( $feed_id ) {
        ...
        if( ( $key = array_search( $feed_id, $feed_queue_ids ) ) !== false ) {
            ...
            update_option( 'rx_insta_feed_media_feed_queue', $feed_queue );
        }
    }
    ...
    public static function clear_feed_queue() {
        update_option( 'rx_insta_feed_media_feed_queue', array() );
    }
    ...
    public static function update_feed_status( $feed_id, $status ) {
        update_post_meta($feed_id, 'rx_insta_feed_status', $status);
    }
}
